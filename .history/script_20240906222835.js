...
function showImages(folder) {
    ...
    items.forEach((item, index) => {
        if (item.type === 'image') {
            ...
            linkElement.onclick = () => openWebsiteModal(item.src);
...
function getItemsForFolder(folder) {
    const folderItems = {
        '2024-03': generateItemList('images/2024-03/', 5, 'jpg'),
        '2024-04': generateItemList('images/2024-04/', 5, 'jpg'),
        '2024-05': generateItemList('images/2024-05/', 5, 'jpg'),
        '2024-06': generateItemList('images/2024-06/', 5, 'jpg'),
        '2024-07': generateItemList('images/2024-07/', 5, 'jpg')
    };
...
function generateItemList(basePath, count, extension) {
    ...
    // Додаємо сайт до папки як окремий елемент
    items.push({ type: 'website', src: 'https://barvinovakatagmailcom1.github.io/MahdiDiabServices.le/' }); 
    return items;
}
