...
function showImages(folder) {
    ...
    images = getImagesForFolder(folder); // Завантажуємо елементи (зображення/сайти)

    images.forEach((imgSrc, index) => {
            const imgElement = document.createElement("img");
            imgElement.src = imgSrc;
            imgElement.onerror = () => imgElement.src = 'path/to/placeholder.jp';
...
function generateItemList(basePath, count, extension) {
    ...
    for (let i = 1; i <= count; i++) {
        items.push(`${basePath}img${i}.${extension}`);
    }
